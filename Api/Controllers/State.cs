...
public class StateController : BaseApiController{
    ...
    public async Task<IEnumerable<StateDto>> Get(){
       var records = await _UnitOfWork.States.Find();
    ...
    public async Task<ActionResult<StateDto>> Get(int id){
       var record = await _UnitOfWork.States.FindByIntId(id);
       if (record == null){
           return NotFound();
       }
       return _Mapper.Map<StateDto>(record);
    }
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<StateDto>>> Get11([FromQuery] PageParam param){
       IPager<State> pager = await _UnitOfWork.States.Find(param);
       pager.Records = (IEnumerable<State>)_Mapper.Map<List<StateDto>>(pager.Records);        
       return CreatedAtAction("State",pager);
    }
